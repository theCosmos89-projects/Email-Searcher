...
Public Class Form1
    ...
    Private bycosmos As String = "Email Searcher v1.0"
    ...
    Private Sub LoadAndApplyCustomFont()
        ...
        Label1.Font = New Font(customFontCollection.Families(0), 55)
    ...
    Private Sub Form1_Shown(sender As Object, e As EventArgs) Handles MyBase.Shown
        ...
        MessageBox.Show("This is a powerful tool; use it responsibly. It searches for emails in your own local files. Only and only in your own files. I hope you find it useful.", "WARNING", MessageBoxButtons.OK, MessageBoxIcon.Warning)
    ...
    Private Sub Finish()
        ...
        Try
            ...
            MessageBox.Show(message, bycosmos & " - Results", MessageBoxButtons.OK, MessageBoxIcon.Information)
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        If Not isOn Then
            ...
            If CheckBox2.Checked = False Then ListBox2.Items.Clear()
            ...
            Try
                ...
                ' Ejecutar ApplyAllFiles en un hilo de background
                Task.Run(Sub()
                             timer.Start()
                             Applyallfiles(epath, ext, runprocess)
                             timer.Stop()

                             ' Al finalizar, volver al hilo de UI para terminar
                             If Me.InvokeRequired Then
                                 Me.Invoke(Sub()
                                               If stopSearch = False Then Finish()
                                           End Sub)
                             Else
                                 If stopSearch = False Then Finish()
                             End If
            ...
            Catch ex As Exception
                MessageBox.Show("An unexpected error occurred while searching", bycosmos & " - Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
                Finish()
                Exit Sub
            End Try
        End If
            ...
            If result = DialogResult.Yes Then
                ...
                For Each selectedIndex As Integer In ListBox1.SelectedIndices
                    If selectedIndex >= 0 AndAlso selectedIndex < ListBox1.Items.Count Then
                        ...
                        FilestoDel.Add(archivoSeleccionado)
                    End If
                Next

                Dim tasks As List(Of Task) = FilestoDel.Select(Function(archivo) Task.Run(Sub()
                                                                                              Try
                                                                                                  File.Delete(archivo)
                ...
                Me.Invoke(Sub()
                              ...
                              For Each archivo As String In FilestoDel
...
End Class
Public Class RedTrackBar
    Inherits TrackBar

    Private pressing As Boolean = False
    ...
    Protected Overrides Sub OnPaint(e As PaintEventArgs)
        ...
        Dim thumbSize As Integer = 15
        Dim trackHeight As Integer = 7

        ' Calcular posición vertical del track
        Dim trackY As Integer = (Me.Height - trackHeight) \ 2

        ' Margen para que el thumb no se salga
        Dim margin As Integer = thumbSize \ 2

        ' Dibujo del track
        Dim trackRect As New Rectangle(margin, trackY, Me.Width - 2 * margin, trackHeight)
        Using trackBrush As New SolidBrush(Color.DarkGray)
            e.Graphics.FillRectangle(trackBrush, trackRect)
        End Using

        ' Calcular posición horizontal del thumb según valor
        Dim usableWidth As Integer = Me.Width - thumbSize
        Dim thumbX As Integer = CInt((Me.Value - Me.Minimum) * usableWidth / (Me.Maximum - Me.Minimum))

        ' Centrar verticalmente el thumb sobre el track
        Dim trackCenterY As Integer = trackY + trackHeight \ 2
        Dim thumbRect As New Rectangle(
        thumbX,
        trackCenterY - thumbSize \ 2,
        thumbSize,
        thumbSize
    )

        ' Color del thumb según si se presiona o no
        Dim thumbColor As Color = If(pressing, Color.DarkRed, Color.Red)

        ' Dibujar el thumb
        Using brush As New SolidBrush(thumbColor)
            e.Graphics.FillEllipse(brush, thumbRect)
        End Using
        Using pen As New Pen(Color.DarkRed, 3)
            e.Graphics.DrawEllipse(pen, thumbRect)
        End Using
    End Sub

    ' Inicializar el thumb en el centro al crear el control
    ...
    Protected Overrides Sub OnHandleCreated(e As EventArgs)
        ...
        ' Inicializar el valor al medio del rango
        Me.Value = (Me.Minimum + Me.Maximum) \ 2
    End Sub
End Class
